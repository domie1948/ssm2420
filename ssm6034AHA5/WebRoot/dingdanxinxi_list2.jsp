...
-->
						...
						<tr align="left" >
							<td colspan="17">
								<form action="dingdanxinxiList2.do" name="myform" method="post">
									查询   订单号：<input name="dingdanhao" type="text" id="dingdanhao" style='border:solid 1px #000000; color:#666666' size="12" /> 支付方式：<select name='zhifufangshi' id='zhifufangshi' style='border:solid 1px #000000; color:#666666;'><option value="">所有</option><option value="微信">微信</option><option value="支付宝">支付宝</option><option value="银行卡">银行卡</option></select>  购买人：<input name="goumairen" type="text" id="goumairen" style='border:solid 1px #000000; color:#666666' size="12" />
    ...
    <td>${u.zhifufangshi}</td>
    <td>${u.goumairen}</td>
    <td>${u.lianxidianhua}</td>
    <td>${u.dizhi}</td>
    
    <td align='center'>${u.issh}
      <c:if test="${u.issh=='否'}"> <input name="button" type="button" onClick="javascript:location.href='sh.jsp?id=${u.id}&amp;yuan=${u.issh}&amp;tablename=dingdanxinxi'" value="支付"/></c:if></td>
                              <td align="center"> ${u.addtime } </td>
                              <td align="center"><a href="doUpdateDingdanxinxi.do?id=${u.id }">编辑</a>  <a href="deleteDingdanxinxi.do?id=${u.id }"
										onclick="{if(confirm('确定要删除吗?')){return true;}return false;}">删除</a> <a href="ddxxDetail.do?id=${u.id }" target="_blank">详细</a></td>
                            </tr>
                          </c:forEach>
                        </table></td>
						</tr>
